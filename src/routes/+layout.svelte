	...
	onMount(() => {
		...
		if (window.performance) {
			...
			if (navEntries.length > 0) {
				...
				latency = Math.round(navEntry.responseStart - navEntry.requestStart) + 'ms';
			}
		}
...
<div class="container">
	...
	<footer>
		<p>
			...
			<!-- @ts-ignore -->
			Services: {__SERVICES_COUNT__} | 
			Server: San Jose, CA
		</p>
		<p>
			Nginx: <span class="ssi">v1.28.1</span> |
			Protocol: <span class="ssi">{protocol}</span> |
			Served: <span class="ssi">{servedTime}</span> |
			Latency: <span class="ssi">{latency}</span>
		</p>
		<p>Hosted with love thanks to <a href="https://heliohost.org" target="_blank" rel="noopener noreferrer">HelioHost</a></p>
